...
namespace IBApi
{
    ...
    public abstract class EClient
    {
        ...
        protected bool VerifyOrder(Order order, int id, bool isBagOrder)
        {
            ...
            {
                ReportError(id, EClientErrors.UPDATE_TWS, " It does not support include overnight parameter.");
                return false;
            }

            if (serverVersion < MinServerVer.MIN_SERVER_VER_CME_TAGGING_FIELDS && order.ManualOrderIndicator != int.MaxValue)
            {
                ReportError(id, EClientErrors.UPDATE_TWS, " It does not support manual order indicator parameter.");
                return false;
            }
